...
- Re-validate commit `4f82018` (single-pass `geometric_mean` + 4-item list hot path + local log/exp binding, lazy shortlist-scorer construction for >cap tasks only, direct `itertools.combinations` in <=cap pruning path, 4-member team-signature fast paths in pruning/swap bounds, 4x4 skill-upper aggregation fast paths, and local-`max` binding in those 4x4 blocks) when host latency returns to a stable band to confirm gains are not regime-specific.
